...
export default function DiamondJourneyPage() {
  ...
  // Only update on entry (not exit) to prevent diamond jumping during fast scroll
  useEffect(() => {
    ...
    const observers = nodes.map((node, idx) => {
      const obs = new IntersectionObserver(
        ([entry]) => {
          if (entry.isIntersecting) setVisibleStageIdx(idx);
        },
        ...
        <div className="max-w-4xl mx-auto">
          <div id="timeline-track" className="relative">
            ...
            {stages.map((stage, i) => {
              ...
              // Stage 5: remove video360Url (shown in stage 6 as Planning Video instead)
              if (stage.number === 5 && displayData) {
                const { video360Url: _v, ...rest5 } = displayData as Record<string, unknown>;
                displayData = rest5;
